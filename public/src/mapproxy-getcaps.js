...
class MapproxyGetCaps extends LitElement {
  ...
  createMapproxyConfig() {
    ...
    const config = {
      ...
      "sources": selectedLayers.reduce((result, layer) => {
        ...
        if (layer.LatLonBoundingBox) {
          if (layer.LatLonBoundingBox[0] < -180 || layer.LatLonBoundingBox[0] > 180 || layer.LatLonBoundingBox[2] < -180 || layer.LatLonBoundingBox[2] > 180 || layer.LatLonBoundingBox[1] < -90 || layer.LatLonBoundingBox[1] > 90 || layer.LatLonBoundingBox[3] < -90 || layer.LatLonBoundingBox[3] > 90) {
            // invalid latlonbbox
            // temp: try 28992
    ...
    this.postMapproxyConfig(localConfig.adminserver, configname, config).then(() => {
